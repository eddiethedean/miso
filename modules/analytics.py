...
def analytics_page():
    ...
    with st.container(border=True):
        ...
        with tab1:
            ...
            for type in types_with_tsoc:
                count, names = get_count_and_names(sum_data_source, type, all_assessments, all_executions, True, "")
                sumArray.append(count)
                sumNamesArray.append(names)
            if include_classifications:
                for type in other_class:
                    count, names = get_count_and_names(sum_data_source, type, all_assessments, all_executions, False, "classification")
                    sumArray.append(count)
                    sumNamesArray.append(names)
            if include_threats:
                for type in other_threats:
                    count, names = get_count_and_names(sum_data_source, type, all_assessments, all_executions, False, "threats")
                    sumArray.append(count)
                    sumNamesArray.append(names)
            if include_miso_program:
                for type in other_miso_program:
                    count, names = get_count_and_names(sum_data_source, type, all_assessments, all_executions, False, "miso_program")
                    sumArray.append(count)
                    sumNamesArray.append(names)
            if include_means:
                for type in other_dissemination_means:
                    count, names = get_count_and_names(sum_data_source, type, all_assessments, all_executions, False, "means")
                    sumArray.append(count)
                    sumNamesArray.append(names)
            for type in other_class_with_tsocs:
                count, names = get_count_and_names(sum_data_source, type, all_assessments, all_executions, True, "classification")
                sumArray.append(count)
                sumNamesArray.append(names)
            for type in other_threats_with_tsocs:
                count, names = get_count_and_names(sum_data_source, type, all_assessments, all_executions, True, "threats")
                sumArray.append(count)
                sumNamesArray.append(names)
            for type in other_miso_program_with_tsocs:
                count, names = get_count_and_names(sum_data_source, type, all_assessments, all_executions, True, "miso_program")
                sumArray.append(count)
                sumNamesArray.append(names)
            for type in other_means_with_tsocs:
                count, names = get_count_and_names(sum_data_source, type, all_assessments, all_executions, True, "means")
                sumArray.append(count)
                sumNamesArray.append(names)
